...
class Rule:
    def __init__(self):
        self.score_directory = os.path.join(os.path.dirname(__file__), "../../asset/score")

        self.start_power = {"ch": 90, "hs": 40, "ep": 25, "ad": 30, "gn": 9000, "cn": 45, "sd": 10}
        self.mid_power = {"ch": 90, "hs": 40, "ep": 25, "ad": 30, "gn": 9000, "cn": 50, "sd": 20}
        self.end_power = {"ch": 100, "hs": 50, "ep": 40, "ad": 40, "gn": 9000, "cn": 40, "sd": 25}

        self.bottom_half_position = {"ch": [], "hs": [], "ep": [], "ad": [], "gn": [], "cn": [], "sd": []}
        for i in self.bottom_half_position.keys():
            with open(os.path.join(self.score_directory, f"{i}.csv"), "r") as f:
                reader = csv.reader(f)
                for row in reader:
                    for r in range(len(row)):
                        row[r] = float(row[r])
                    self.bottom_half_position[i] += [row]

        self.upper_half_position = {"ch": [], "hs": [], "ep": [], "ad": [], "gn": [], "cn": [], "sd": []}
        for i in self.bottom_half_position.keys():
            self.upper_half_position[i] = self.bottom_half_position[i][::-1]

    def chariot_valid_move_list(self, board, position, red_is_machine):
        valid_move_list = []
    ...
    def horse_valid_move_list(self, board, position, red_is_machine):
        valid_move_list = []
    ...
    def elephant_valid_move_list(self, board, position, red_is_machine):
        valid_move_list = []
        side = board[position[0]][position[1]][0]
        row = position[0]
        col = position[1]
    ...
    def advisor_valid_move_list(self, board, position, red_is_machine):
        valid_move_list = []
        row = position[0]
        col = position[1]
        side = board[position[0]][position[1]][0]
    ...
    def general_valid_move_list(self, board, position, red_is_machine):
        valid_move_list = []
        row = position[0]
        col = position[1]
        chess_side = board[position[0]][position[1]][0]
    ...
    def cannon_valid_move_list(self, board, position, red_is_machine):
        valid_move_list = []
        row = position[0]
        j = position[1]
        chess_side = board[position[0]][position[1]][0]
    ...
    def soldier_valid_move_list(self, board, position, red_is_machine):
        valid_move_list = []
        row = position[0]
        col = position[1]
        chess_side = board[position[0]][position[1]][0]
    ...
    def move_rule(self, board, position, red_is_machine):
        piece_funcs = {
            "ch": self.chariot_valid_move_list,
            "hs": self.horse_valid_move_list,
            "cn": self.cannon_valid_move_list,
            "gn": self.general_valid_move_list,
            "ad": self.advisor_valid_move_list,
            "sd": self.soldier_valid_move_list,
            "ep": self.elephant_valid_move_list,
        }
    ...
    def move_check_valid(self, board, red_turn, red_is_machine):
        check = False
        black_general, red_general = self.find_generals(board)
        if red_is_machine:
        ...
        if black_general[1] == red_general[1]:
            ...
            if check:
                return False
        if self.is_checked(board, black_general, red_general, not red_turn, red_is_machine):
            return False
        return True

    def check_illegal_move(self, game_state):
        black_general, red_general = self.find_generals(game_state.board)
        if self.is_checked(
            game_state.board,
            black_general,
            red_general,
    ...
    def is_checked(self, board, black_general, red_general, red_turn, red_is_machine):
        x = black_general[0]
        y = black_general[1]
        chess_side = "b"
        ...
        if horse_position_list != []:
            ...
            for i in horse_position_list:
                if i in candidate_general_threaten_list:
                    valid_horse_threat_list = self.horse_valid_move_list(board, (i[0], i[1]), red_is_machine)
        ...
        if cannon_position_list != []:
            ...
            for i in cannon_position_list:
                if i in candidate_general_threaten_list:
                    valid_canon_threat_list = self.cannon_valid_move_list(board, i, red_is_machine)
        ...
        if soldier_position_list != []:
            ...
            for i in soldier_position_list:
                if i in candidate_general_threaten_list:
                    valid_soldier_threat_list = self.soldier_valid_move_list(board, i, red_is_machine)
